  ...
  async createSuperAdmin(): Promise<void> {
    ...
    if (!existingAdmin) {
      ...
      const superAdmin = this.usuariosRepository.create({
        username: 'super@dmin', // Asegúrate que 'username' existe en Usuarios entity
        password: hashedPassword, // Cambia esto por un hash en producción
        correo: 'admin@libroseureka.com.pe', // Cambia esto por el correo electrónico del superadmin
        nombres: 'Super', // Cambia esto por el nombre del superadmin (ajustado a 'nombres')
